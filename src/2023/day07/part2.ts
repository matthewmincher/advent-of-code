...
class Hand {
  ...
  constructor(cards: string[], bid: number) {
    ...
    if (jokerCount === 5) {
      ...
      let largestDistinctCard: string | null = null;
      let largestDistinct = 0;
      for (const [card, count] of Object.entries(this.sets)) {
        if (count > largestDistinct) {
          largestDistinct = count;
          largestDistinctCard = card;
        }
      }

      this.sets[largestDistinctCard!] += jokerCount;
    }
  }
